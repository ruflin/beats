...
if __name__ == "__main__":
    ...
    parser.add_argument("--fields", help="Path to fields file")
    parser.add_argument("--target", help="Path to traget template file")
    #parser.add_argument("es_beats", help="The path to the general beats folder")

    args = parser.parse_args()
    target = args.target
    fields_yml = args.fields
    #target = "./nginx/access/template.json"
#
    #fields_yml = "./nginx/access/_meta/fields.yml"

    with open(fields_yml, 'r') as f:
        fields = f.read()

        with open(target, 'w') as output:
            fields_to_es_template(args, fields, output, "testoutput-*", "0.0.1")
